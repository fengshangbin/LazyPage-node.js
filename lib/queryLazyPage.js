...
function addLevelMark(html) {
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    html = html.substring(0, item.lazypageIndex + addCount) + mark + html.substring(item.lazypageIndex + addCount);
    addCount += mark.length;
  }
  return html;
}
function removeLevelMark(html) {
  return html.replace(/ lazypagelevel\d/g, '');
}
function queryLazyPageElement(html) {
  ...
  while (group != null) {
    ...
    var lazypageIndex = group.index + group[0].search(group[2]) + classIndex + 'lazypage'.length;
    var searchStart = group.index + group[0].length;
    var closeIndex = 0;
    if (/\/ *>$/.test(group[0]) == false) {
      closeIndex = queryHelp.queryCloseTag(group[1], html.substring(searchStart));
    }
    var lazyPage = {
      start: group.index,
