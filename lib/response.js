...
var queryLazyPageSelector = require('./queryLazyPage');
    ...
    if (ext != 'unknown' && ext != 'html') {
      ...
      if (hitHtml) {
        if (root && typeof root == 'string') {
          ...
          lazypage(req, res, assets[path].source(), cookies, next);
        }
      } else {
        next();
      }
...
function lazypage(req, res, html, cookies, next) {
  var fullUrl = req.protocol + '://' + req.get('host') + req.originalUrl.replace('//', '/');
  //console.log(fullUrl);
  var urls = fullUrl.split('?');
  //query = query.length > 1 ? query[1] : null;
  try {
    //console.log(mapping, urls[0], urls[1]);
    new analyzeHtml().parse(mapping, ignorePath, urls[0], urls[1], html, cookies, function(code, result) {
      if (code == 200) {
