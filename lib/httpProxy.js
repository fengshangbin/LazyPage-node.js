...
function ajax(rootPath, paths, method, urlString, parameters, cookies, callback) {
	...
	const req = httpClient.request(options, (res) => {
	  ...
	  if(res.statusCode>200){
		  console.error(`error on ajax ${urlString}: statusCode ${res.statusCode}`);
		  callback(null);
	  }else{
	...
	req.on('error', (e) => {
	    console.error(`error on ajax ${urlString}: ${e.message}`);
		callback(null);
	});
