...
function queryElement(regStr, parent, option) {
  ...
  while (group != null) {
    ...
    var el = dom.findElement(group.index);
    if (el == null) {
      ...
      el = new element(dom, group.index + parentStart, searchStart + closeIndex + parentStart, targetHtml, group[0].length, closeLen);
      dom.addElement(el);
    }
    if (result == null) {
      result = el;
  ...
  while (openCount > 0) {
    ...
    if (groupAll == null) {
      ...
      if (matchClose.test(groupAll[0])) {
        ...
        openCount++;
        if (new RegExp('\\b' + tag + '\\b', 'i').test('input br image')) return { index: 0, len: 0 };
      }
    }
  }
